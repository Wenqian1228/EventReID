...
class NetworkBasic(torch.nn.Module):
    def __init__(self, netParams,
        ...
        self.conv2 = self.slayer2.conv(8, 64, 3, padding=1)
        ...
        self.offsets2 = nn.Conv2d(64, 18, kernel_size=3, padding=1)
        self.deform2 = DeformConv2D(64,256,kernel_size=3,padding=1)

        self.offsets3 = nn.Conv2d(256, 18, kernel_size=3, padding=1)
        self.deform3 = DeformConv2D(256,512,kernel_size=3,padding=1)
    ...
    def forward(self, spikeInput):
        ...
        spikes_layer_2 = self.slayer2.spike(self.conv2(spikes_layer_1)) # [16,64,3,128,64]
        print('spikes_layer_2=',spikes_layer_2.shape)
        print('spikes_layer_2=',spikes_layer_2[:,:,0,:,:].shape)

        b, t, c, w, h = spikes_layer_2.size()    # [16,8,3,256,128] [b, t, c, w, h]
        x = spikes_layer_2.contiguous().view(b * t, c, w, h)  # x=[128,3,256,128]
        # x = F.interpolate(x,scale_factor=0.5)

        # offset1_x = self.offsets1(x)
        # x = self.deform1(x,offset1_x)
        # x = F.interpolate(deform1_x,scale_factor=0.5)   # [128,64,128,64]
        # print('deform1_x=',deform1_x.shape)
        
        # x = self.res_conv1(x)   # [128,64,64,32]
        # x = self.bn1(x)
        # x = self.relu(x)
        # x = self.maxpool(x)     # [128,64,32,16]




        # x = self.layer1(x)      # [128,256,32,16]
        # # offsets2 = self.offsets2(x)
        # # x = self.deform2(x,offsets2)    # [128,256,32,16]

        # # offsets3 = self.offsets3(x)
        # # x = self.deform3(x,offsets3)    # [128,256,32,16]
        # # x = F.interpolate(x,scale_factor=0.5) 
        
        # x = self.layer2(x)      # [128,512,16,8]
        # # print('self.layer2=',x.shape)
        # x = self.layer3(x)      # [128,1024,8,4]
        # # print('self.layer3=',x.shape)
        # x = self.layer4(x)      # [128,2048,8,4]
        # print('self.layer4=',x.shape)

        return x    # [128,2048,8,4]

#############################################################################
# netParams = snn.params('network.yaml')
_input = torch.zeros((16,16,3,128,64))
_input = _input.cuda()
