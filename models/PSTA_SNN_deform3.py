...
# 只有img数据进行re-id，虽然event参与特征提取，但是没有实际意义
################################################################################################
class PSTA_SNN_deform3(nn.Module):

    def __init__(self, num_classes, model_name, pretrain_choice, seq_len=8):
        super(PSTA_SNN_deform3, self).__init__()
        self.inplanes = 64
        self.in_planes = 2048
        self.base = ResNet()
        ...
        self.deformconv1 = DeformConv2D(3, 3, kernel_size=3, padding=1)

        self.offsets2 = nn.Conv2d(3, 18, kernel_size=3, padding=1)
        self.deformconv2 = DeformConv2D(3, 3, kernel_size=3, padding=1)

        self.offsets3 = nn.Conv2d(3, 18, kernel_size=3, padding=1)
        self.deformconv3 = DeformConv2D(3, 3, kernel_size=3, padding=1)

        self.offsets4 = nn.Conv2d(3, 18, kernel_size=3, padding=1)
        self.deformconv4 = DeformConv2D(3, 3, kernel_size=3, padding=1)
        ...
        self.res_layer2 = self._make_layer(block,128,layers[1],stride=2)
        self.res_layer3 = self._make_layer(block,256,layers[2],stride=2)
        self.res_layer4 = self._make_layer(block,512,layers[3],stride=1)
    ...
    def forward(self, x, pids=None, camid=None):    # x=[16,8,3,256,128]
        ...
        event = F.relu(self.deformconv(event, offsets))     # x = [64,3,256,128] feat_map= [128, 2048, 16, 8]
        
        # event = self.res_conv1(event)
        # event = self.res_layer1(event)
        # event = self.res_layer2(event)
        # event = self.res_layer3(event)
        # event_feat_map = self.res_layer4(event)
        ...
        feat_map = feat_map + event_feat_map * 0.00000001
        # b,c,w,h = feat_map.size()
        # event_feature_map = feat_map[b//2:,:,:,:]
        # feat_map = feat_map[0:b//2,:,:,:]
        # b,c,w,h = feat_map.size()
        w = feat_map.size(2)
        h = feat_map.size(3)
