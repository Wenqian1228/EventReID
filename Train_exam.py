...
parser.add_argument('--arch', type=str, default='OSNet_visual', choices=['ResNet50', 'PSTA', 'HiCMD_img', 'OSNet_img', 'SRS_img', 'STMN_img', 'TransReID_img','CCReID_img','MCNL_img','GRL_img','STGCN_img','SINet','BiCNet','TCLNet','AP3D_img','PSTA_visual','OSNet_visual'])
...
parser.add_argument('--dataset', type=str, default='PRID_mat_img2', choices=['Low','duke','prid','PRID_mat_img','PRID_mat_img2','PRID_block','PRID_blur9_10','Low','prid_dark2','iLIDSVID','iLIDSVID_dark','iLIDSVID_mat_img'])
...
def main():
    ...
    # 数据集的初始化操作
    if args_.dataset == 'prid' or args_.dataset == 'prid_dark' or args_.dataset == 'PRID_mat_img' or args_.dataset == 'PRID_mat_img2' or args_.dataset == 'prid_dark2' or args_.dataset == 'Low' or args_.dataset == 'prid_dark2_SCI' or args_.dataset == 'PRID_blur9_10' or args_.dataset == 'PRID_blur19_20' or args_.dataset == 'PRID_blur29_30' or args_.dataset == 'PRID_block':
...
def train(model, trainloader, xent, tent, optimizer, use_gpu):
    ...
    for batch_idx, (imgs, pids, _, _) in enumerate(trainloader):
        ...
        if use_gpu:
            imgs = imgs.cuda()
            pids = pids.cuda()
        outputs, features = model(imgs)     # imgs=[16,8,3,256,128] [b, t, c, w, h]
