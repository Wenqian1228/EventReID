...
import data_manager
from samplers_visual import RandomIdentitySampler
from video_loader import VideoDataset

from video_event_loader import Video_Event_Dataset
from video_event_img_loader import Video_Event_img_Dataset
...
parser.add_argument("--config_file", default="./configs/softmax_triplet_prid128_visual.yml", help="path to config file", type=str)
...
parser.add_argument('--dataset', type=str, default='PRID_dark_event_img_vid', choices=['mars','duke','prid','prid_event','prid_event_vid','mars_event_vid','prid_dark2_event_vid','Low_event_vid','iLIDSVID_event_vid','iLIDSVID_event_vid_blur4_5','iLIDSVID_block_event','PRID_blur_event','iLIDSVID_dark_event','PRID_event_img_vid','PRID_block','Low','PRID_dark_event_img_vid'])
...
def main():
    ...
    # 数据集的初始化操作
    if args_.dataset == 'prid' or args_.dataset == 'prid_event' or args_.dataset =='prid_event_vid' or args_.dataset =='prid_dark2_event_vid' or args_.dataset =='Low_event_vid'  or args_.dataset =='PRID_blur_event' or args_.dataset =='PRID_event_img_vid' or args_.dataset =='PRID_block' or args_.dataset ==  'PRID_dark_event_img_vid':
...
##################################################################################
    ...
    if args_.dataset == 'mars_event_vid':
        ...
        trainloader = DataLoader(
            Video_Event_img_Dataset(dataset.train, dataset.train_event, seq_len=args_.seq_len, sample=args_.train_sampler, transform=transform_train,
                        dataset_name=args_.dataset),
            sampler=video_sampler,
            batch_size=cfg.SOLVER.SEQS_PER_BATCH, num_workers=cfg.DATALOADER.NUM_WORKERS,
            pin_memory=pin_memory, drop_last=True
        ...
        if args_.test_sampler == 'dense':
            print('Build dense sampler')
            queryloader = DataLoader(
                Video_Event_img_Dataset(dataset.query, dataset.query_event, seq_len=args_.seq_len, sample=args_.test_sampler, transform=transform_test,
                            max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
                batch_size=1 , shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
                pin_memory=pin_memory, drop_last=False
            )
            ...
            galleryloader = DataLoader(
                Video_Event_img_Dataset(dataset.gallery, dataset.gallery_event, seq_len=args_.seq_len, sample=args_.test_sampler, transform=transform_test,
                            max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
                batch_size=1 , shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
                pin_memory=pin_memory, drop_last=False,
            )
            ...
            queryloader = DataLoader(
                Video_Event_img_Dataset(dataset.query, dataset.query_event, seq_len=args_.seq_len, sample=args_.test_sampler,
                            transform=transform_test,
                            max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
                batch_size=cfg.TEST.SEQS_PER_BATCH, shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
                pin_memory=pin_memory, drop_last=False
            ...
            galleryloader = DataLoader(
                Video_Event_img_Dataset(dataset.gallery,dataset.gallery_event, seq_len=args_.seq_len, sample=args_.test_sampler,
                            transform=transform_test,
                            max_seq_len=cfg.DATASETS.TEST_MAX_SEQ_NUM, dataset_name=cfg.DATASETS.NAME),
                batch_size=cfg.TEST.SEQS_PER_BATCH, shuffle=False, num_workers=cfg.DATALOADER.NUM_WORKERS,
                pin_memory=pin_memory, drop_last=False,
